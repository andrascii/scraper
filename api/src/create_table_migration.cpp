...
CreateTableMigration::CreateTableMigration(
  ...
  Params params
) : AbstractDbMigration{migration_id}, params_{std::move(params)} {
...
std::string CreateTableMigration::BuildSqlQuery() const {
  ...
  std::for_each(
    begin(params_.foreign_keys),
    end(params_.foreign_keys),
  ...
  std::for_each(
    begin(params_.unique_keys),
    end(params_.unique_keys),
