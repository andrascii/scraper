...
std::string AlterTableMigration::BuildSqlQuery() const {
  ...
  std::for_each(
    begin(params_.drop_columns), end(params_.drop_columns), [&](const std::string& column) {
  ...
  std::for_each(
    begin(params_.add_columns), end(params_.add_columns), [&](const ColumnDefinition& column) {
  ...
  std::for_each(
    begin(params_.add_foreign_keys), end(params_.add_foreign_keys), [&](const ForeignKey& key) {
  ...
  std::for_each(
    begin(params_.drop_unique_keys), end(params_.drop_unique_keys), [&](const UniqueKey& key) {
  ...
  std::for_each(
    begin(params_.add_unique_keys), end(params_.add_unique_keys), [&](const UniqueKey& key) {
  ...
  sql_operations.erase(
    std::remove_if(
      begin(sql_operations), end(sql_operations), [](const std::string& str) {
        return str.empty();
      }
    ), end(sql_operations)
  );
