...
Application::Application(
  ...
  http_handler_registry_->AddPostHandler(PostHttpHandlerType::kAddJob, std::make_shared<AddJobHandler>(pg_pool_));
  http_handler_registry_->AddPostHandler(PostHttpHandlerType::kRemoveJob, std::make_shared<RemoveJobHandler>(pg_pool_));
  http_handler_registry_->AddPostHandler(PostHttpHandlerType::kEnableJob, std::make_shared<EnableJobHandler>(pg_pool_));
  http_handler_registry_->AddPostHandler(PostHttpHandlerType::kDisableJob, std::make_shared<DisableJobHandler>(pg_pool_));
  http_handler_registry_->AddPostHandler(PostHttpHandlerType::kUpdateJob, std::make_shared<UpdateJobHandler>(pg_pool_));
  http_handler_registry_->AddPostHandler(PostHttpHandlerType::kJobList, std::make_shared<JobListHandler>(pg_pool_));
}
...
std::error_code Application::StartHttpServer() {
  ...
  std::make_shared<Listener>(ctx_, boost::asio::ip::tcp::endpoint{address, port}, http_handler_registry_)->Run();
