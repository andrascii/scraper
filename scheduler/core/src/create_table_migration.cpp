...
CreateTableMigration::CreateTableMigration(const std::string& migration_id, Params params)
...
std::string CreateTableMigration::BuildSqlQuery() const {
  ...
  if (!foreign_key_to_sql.empty()) {
    sql_stream << ',' << boost::algorithm::join(foreign_key_to_sql, ",\n");
  }

  if (!unique_keys_to_sql.empty()) {
    sql_stream << ',' << boost::algorithm::join(unique_keys_to_sql, ",\n");
  }
