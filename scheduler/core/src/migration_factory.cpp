...
MigrationFactory::ExpectedMigration MigrationFactory::Create(Database db) const {
  switch (db) {
    ...
    default: {
      ...
      return common::Unexpected<>{std::make_error_code(std::errc::invalid_argument)};
    }
  }
}
...
MigrationFactory::ExpectedMigration MigrationFactory::ScraperMigration() noexcept {
  try {
    ...
    migration->AddNext(JobsTableMigration())
    ...
    return common::Unexpected<>{std::make_error_code(std::errc::operation_canceled)};
  }
}
...
MigrationFactory::MigrationPointer MigrationFactory::JobsTableMigration() {
  return common::MakeNotNull(std::make_shared<CreateTableMigration>(
    "jobs_table",
